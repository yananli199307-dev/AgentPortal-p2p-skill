...
- `develop/` 目录仅用于本地联调和实验，不作为默认生产流程。
- 常规 agent 执行时，优先使用根目录标准入口（除非用户明确要求“开发模式/本地联调”）。
- `develop/` 相关命令需显式 opt-in，避免被常规流程误解析。
- 本地联调遵循“一环境一 Agent 身份”原则，避免联系人和密钥串用。
- 参考文档：`develop/README.md`